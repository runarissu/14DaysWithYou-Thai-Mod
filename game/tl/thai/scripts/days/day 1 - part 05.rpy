# TODO: Translation updated at 2026-09-05 03:10

# game/scripts/days/day 1.rpy:46
translate thai day1_rejectren_48d00640:

    # n "Back at home, the sounds of my microwave meal and the television filled the silence as I told [ch_moth] about my day."
    n "กลับถึงบ้าน เสียงไมโครเวฟอาหารและทีวีดังกังวานแทนความเงียบ ขณะที่ฉันเล่าเรื่องวันนี้ให้ [ch_moth] ฟัง"

# game/scripts/days/day 1.rpy:1630
translate thai day1_deadendstart_ccaaaa7f:

    # n "I climb into bed with the thought of the supposed intruder still heavy on my mind, but I eventually put it to rest by remembering the heavy duty lock I bought and installed earlier."
    n "ฉันขึ้นเต้ายายความสงสัยเรื่องคนบุกรุกยังวนเวียนอยู่ในหัว แต่สุดท้ายก็โล่งใจได้เพราะนึกได้ว่าล็อกแน่นที่ซื้อและติดตั้งไว้ก่อนหน้านี้"

# game/scripts/days/day 1.rpy:1982
translate thai day1_deadendstart_18279e6f:

    # n "There's no way anyone could break in now, and even if they did, I could rely on my neighbour to hear."
    n "คนไม่มีทางบุกเข้ามาได้แน่ และถ้าเกิดมาจริง ฉันก็พึ่งเพื่อนบ้านให้ช่วยเหลือได้"

# game/scripts/days/day 1.rpy:1983
translate thai day1_deadendstart_1b389e6e:

    # n "Often, I'd hear [ch_violet] gaming away on her computer until the early hours of the morning, so I felt safe knowing that she'd be around if something went wrong. "
    n "ฉันมักจะได้ยินเสียง [ch_violet] เล่นเกมอยู่หน้าคอมจนดึกดื่น ฉันเลยรู้สึกปลอดภัยที่รู้ว่าเธอจะอยู่ใกล้ๆ ถ้ามีอะไรผิดปกติ "

# game/scripts/days/day 1.rpy:1984
translate thai day1_deadendstart_eabec665:

    # n "…"
    n "…"

# game/scripts/days/day 1.rpy:1985
translate thai day1_deadendstart_0d5b4789:

    # n "…Even if I couldn't hear her right now."
    n "…ถึงแม้ตอนนี้ฉันจะไม่ได้ยินเสียงเธอก็ตาม"

# game/scripts/days/day 1.rpy:1986
translate thai day1_deadendstart_603dac86:

    # y "Oh well. I'm sure everyone takes gaming breaks every so often."
    y "เอาน่า คนเราก็พักเล่นเกมบ้างเป็นบางครั้ง"

# game/scripts/days/day 1.rpy:1988
translate thai day1_deadendstart_811878af:

    # n "Snuggling deeper into the warmth of my blankets, my eyes began to feel heavy as I slowly lulled myself into dreamland."
    n "ฉันซุกตัวลึกเข้าไปในความอบอุ่นของผ้าห่ม ตาเริ่มหนักอึ้งขณะที่ค่อยๆ หลุดเข้าสู่แดนฝัน"

# game/scripts/days/day 1.rpy:1993
translate thai day1_deadendstart_f90c9f5d:

    # n "Because of this, I miss \ \ {color=#a30b11}{size=-6}01100001{/size}{/color} \ \ \ \ {color=#a30b11}{size=-6}01101110{/size}{/color} \ \ \ \ the sounds of foot \ \ \ \ {color=#a30b11}{size=-6}01100111{/size}{/color} \ \ steps creeping \ \ \ \ \ \ {color=#a30b11}{size=-6}01100101{/size}{/color} \ \ \ \ \ down the hall \ \ \ \ \ \ \ \ \ \ \ {color=#a30b11}{size=-6}01101100{/size}{/color} \ way {color=#a30b11} \ \ . \ \ \ \ \ \ \ \ \ \ . \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ .{/color}"
    n "เพราะเช่นนั้น ฉันจึงพลาด \ \ {color=#a30b11}{size=-6}01100001{/size}{/color} \ \ \ \ {color=#a30b11}{size=-6}01101110{/size}{/color} \ \ \ \ เสียงฝีเท้า \ \ \ \ {color=#a30b11}{size=-6}01100111{/size}{/color} \ \ ที่ย่อง \ \ \ \ \ \ {color=#a30b11}{size=-6}01100101{/size}{/color} \ \ \ \ \ ลัดเลาะ \ \ \ \ \ \ \ \ \ \ \ {color=#a30b11}{size=-6}01101100{/size}{/color} \ ทาง {color=#a30b11} \ \ . \ \ \ \ \ \ \ \ \ \ . \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ .{/color}"
